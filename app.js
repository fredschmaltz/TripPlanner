...
return `<div class="doc-item ${isLoaded ? 'loaded' : 'missing'}" onclick="clickDocItem('${filepath.replace(/'/g, "\\'")}', ${isLoaded})"> <span class="doc-status">${isLoaded ? '✓' : '○'}</span> <span class="doc-name" title="${basename}">${basename}</span> ${folder ?`<span class="doc-folder">${folder}</span>` : ''} </div>`;
}).join('');
...
window.edRenameRouteCityDebounced = edRenameRouteCityDebounced;
