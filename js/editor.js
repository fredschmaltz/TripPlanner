...
return ` <div class="editor-inline-row editor-route-stop"> <input type="color" class="editor-color-input editor-route-color" value="${r.color || '#aaaaaa'}" oninput="editorData.trip.route[${i}].color=this.value;edSyncRouteColorToDays(${i})"> <select class="editor-inline-input editor-flag-select" onchange="editorData.trip.route[${i}].flag=this.value;edSyncRouteFlagToDays(${i});refreshSection('route')"> <option value="">${t('ed.selectCountry')}</option> ${currentFlagOptions} </select> <div class="editor-loc-wrap" id="route-city-wrap-${i}" style="flex:1;min-width:0"> <div class="editor-loc-input-row"> <input class="editor-input editor-loc-query" id="${qId}" value="${escHtml(r.city || '')}" placeholder="${t('ed.cityName')}" oninput="edRenameRouteCityDebounced(${i},this.value)" onkeydown="if(event.key==='Enter'){event.preventDefault();edSearchRouteCity(${i},this.value)}"> ${coordBadge} <button class="editor-loc-search-btn" onclick="edSearchRouteCity(${i},document.getElementById('${qId}').value)" title="${t('ed.locSearch')}">${icon('search',14)}</button> </div> <div class="editor-loc-results" id="${rId}"></div> </div> <div class="editor-route-actions-inline"> <button class="editor-move-btn" onclick="edMoveRoute(${i},-1)" title="${t('ed.moveUp')}">↑</button> <button class="editor-move-btn" onclick="edMoveRoute(${i},1)" title="${t('ed.moveDown')}">↓</button> <button class="editor-remove-btn" onclick="edRemoveRoute(${i})" title="${t('ed.remove')}">✕</button> </div> </div>`;
}).join(’’);
...
`<div class="editor-loc-result" onclick="edPickRouteCityResult(${routeIdx},${i})"> <span class="editor-loc-result-name">${escHtml(r.label.split(',').slice(0, 3).join(','))}</span> <span class="editor-loc-result-coord">${r.lat.toFixed(4)}, ${r.lng.toFixed(4)}</span> </div>`
).join(’’);
} catch {
...
const certsHTML = (ins.certificates || []).map((c, i) => ` <div class="editor-badge-sub-item"> <input class="editor-inline-input flag-input" value="${c.flag || ''}" oninput="editorData.trip.insurance.certificates[${i}].flag=this.value" placeholder="XX"> <input class="editor-inline-input" style="flex:1" value="${escHtml(c.title || '')}" oninput="editorData.trip.insurance.certificates[${i}].title=this.value" placeholder="${t('ed.title')}"> <input class="editor-inline-input" style="flex:2" value="${escHtml(c.file || '')}" oninput="editorData.trip.insurance.certificates[${i}].file=this.value" placeholder="Insurance/file.pdf"> <button class="editor-remove-btn" onclick="editorData.trip.insurance.certificates.splice(${i},1);refreshSection('badges')">✕</button> </div>`).join(’’);
...
const docsHTML = (cus.documents || []).map((d, i) => ` <div class="editor-badge-sub-item"> <input class="editor-inline-input flag-input" value="${d.flag || ''}" oninput="editorData.trip.customs.documents[${i}].flag=this.value" placeholder="XX"> <input class="editor-inline-input" style="flex:1" value="${escHtml(d.title || '')}" oninput="editorData.trip.customs.documents[${i}].title=this.value" placeholder="${t('ed.title')}"> <input class="editor-inline-input" style="flex:2" value="${escHtml(d.file || '')}" oninput="editorData.trip.customs.documents[${i}].file=this.value" placeholder="Customs/file.pdf"> <button class="editor-remove-btn" onclick="editorData.trip.customs.documents.splice(${i},1);refreshSection('badges')">✕</button> </div>`).join(’’);
...
let itemsHTML = ‘’;
if (!isSimple) {
itemsHTML = (b.items || []).map((item, ii) => ` <div class="editor-badge-sub-item"> <input class="editor-inline-input flag-input" value="${item.flag || ''}" oninput="editorData.trip.customBadges[${idx}].items[${ii}].flag=this.value" placeholder="XX"> <input class="editor-inline-input" style="flex:1" value="${escHtml(item.title || '')}" oninput="editorData.trip.customBadges[${idx}].items[${ii}].title=this.value" placeholder="${t('ed.title')}"> <input class="editor-inline-input" style="flex:2" value="${escHtml(item.file || '')}" oninput="editorData.trip.customBadges[${idx}].items[${ii}].file=this.value" placeholder="folder/file.pdf"> <button class="editor-remove-btn" onclick="editorData.trip.customBadges[${idx}].items.splice(${ii},1);refreshSection('badges')">✕</button> </div>`).join(’’);
}

return `<div class="editor-badge-item"> <div class="editor-badge-header"> <input class="editor-inline-input" style="width:40px;text-align:center;font-size:1rem" value="${b.icon || ''}" oninput="editorData.trip.customBadges[${idx}].icon=this.value" placeholder="icon"> <input class="editor-inline-input" style="flex:1;font-weight:600" value="${escHtml(b.label || '')}" oninput="editorData.trip.customBadges[${idx}].label=this.value" placeholder="${t('ed.badgeName')}"> <button class="editor-remove-btn" onclick="editorData.trip.customBadges.splice(${idx},1);refreshSection('badges')">✕</button> </div> <div class="editor-row"> <div class="editor-field"><label class="editor-label">${t('ed.borderColor')}</label> <input type="color" class="editor-color-input" value="${b.borderColor || '#32333f'}" oninput="editorData.trip.customBadges[${idx}].borderColor=this.value"> </div> <div class="editor-field"><label class="editor-label">${t('ed.type')}</label> <select class="editor-select" onchange="editorData.trip.customBadges[${idx}].type=this.value;refreshSection('badges')"> <option value="simple" ${isSimple ? 'selected' : ''}>${t('ed.simpleType')}</option> <option value="multi" ${!isSimple ? 'selected' : ''}>${t('ed.multiType')}</option> </select> </div> </div> ${isSimple ? `
<div class="editor-row">
...
// ── Day basic fields ──
let html = `<div class="editor-row"> <div class="editor-field" style="flex:2"> <label class="editor-label">${day.dayTrip ? t('ed.baseCity') : t('ed.city')}</label> <div class="editor-city-select-wrap"> <span class="editor-city-dot" style="background:${currentColor}"></span> <select class="editor-select editor-city-select" onchange="edSelectDayCity(${idx},this.value)"> <option value="">${t('ed.selectCity')}</option> ${cityOptions} </select> </div> </div> <div class="editor-field" style="flex:1"> <label class="editor-label">${t('ed.date')}</label> <input type="date" class="editor-input" value="${day.dateISO || ''}" onchange="edSetDayDate(${idx},this.value)"> </div> </div> <div class="editor-row"> <div class="editor-field"> <label class="editor-label" style="display:flex;align-items:center;gap:6px"> <input type="checkbox" ${day.dayTrip ? 'checked' : ''} onchange="edToggleDayTrip(${idx},this.checked)"> ${t('ed.dayTrip')} </label> </div> ${day.dayTrip ?`<div class="editor-field"><label class="editor-label">${t(‘ed.destination’)}</label><input class="editor-input" value="${escHtml(day.city || '')}" oninput="editorData.days[${idx}].city=this.value" placeholder="${t('ph.nara')}"></div>` : ''} </div>`;

// ── Food ──
html += `<div class="editor-subsection"> <div class="editor-subsection-header"> <span class="editor-subsection-title">${icon('utensils',13)} ${t('ed.foodTips')}</span> <span class="editor-subsection-count">${(day.food || []).length}</span> </div>`;
...
html += `<div class="editor-food-row"> <select class="editor-emoji-select" onchange="edEmojiSelect(this,'editorData.days[${idx}].food[${fi}].e',${idx})"> <option value="">---</option> ${foodIconOpts} <option value="__custom__" ${isCustomFood ? 'selected' : ''}>${isCustomFood ? f.e + ' ' : ''}${t('ed.customOpt')}</option> </select> <div style="flex:1;display:flex;flex-direction:column;gap:4px"> <input class="editor-inline-input" value="${escHtml(f.dish || '')}" oninput="editorData.days[${idx}].food[${fi}].dish=this.value" placeholder="${t('ed.dishName')}"> <input class="editor-inline-input" value="${escHtml(f.desc || '')}" oninput="editorData.days[${idx}].food[${fi}].desc=this.value" placeholder="${t('ed.description')}" style="font-size:0.75rem;color:var(--muted)"> </div> <button class="editor-remove-btn" onclick="editorData.days[${idx}].food.splice(${fi},1);refreshDayBody(${idx})">✕</button> </div>`;
});
...
// ── Files ──
html += `<div class="editor-subsection"> <div class="editor-subsection-header"> <span class="editor-subsection-title">${icon('paperclip',13)} ${t('ed.files')}</span> <span class="editor-subsection-count">${(day.files || []).length}</span> </div>`;
(day.files || []).forEach((f, fi) => {
const catOpts = Object.keys(CAT_LABEL).map(k => `<option value="${k}" ${f.cat === k ? 'selected' : ''}>${CAT_LABEL[k]}</option>`).join(’’);
html += `<div class="editor-file-row"> <select class="editor-inline-input" style="width:110px" onchange="editorData.days[${idx}].files[${fi}].cat=this.value">${catOpts}</select> <input class="editor-inline-input" style="flex:2" value="${escHtml(f.name || '')}" oninput="editorData.days[${idx}].files[${fi}].name=this.value" placeholder="${t('ed.filename')}"> <input class="editor-inline-input" style="flex:2" value="${escHtml(f.label || '')}" oninput="editorData.days[${idx}].files[${fi}].label=this.value" placeholder="${t('ed.displayLabel')}"> <button class="editor-add-btn" style="margin:0;padding:4px 8px;font-size:0.7rem" onclick="edPickFile(${idx},${fi})">${icon('folder',12)}</button> <button class="editor-remove-btn" onclick="editorData.days[${idx}].files.splice(${fi},1);refreshDayBody(${idx})">✕</button> </div>`;
});
...
// ── Cards ──
html += `<div class="editor-subsection"> <div class="editor-subsection-header"> <span class="editor-subsection-title">${icon('clipboard',13)} ${t('ed.cardsSection')}</span> <span class="editor-subsection-count">${(day.cards || []).length}</span> </div>`;
...
let html = ` <div class="editor-row"> <div class="editor-field" style="flex:12"> <label class="editor-label">${t('ed.type')}</label> <select class="editor-select" onchange="edSelectCardType(${dayIdx},${cardIdx},this.value)">${typeOptions.replace('value="' + c.type + '"', 'value="' + c.type + '" selected')}</select> </div> <div class="editor-field" style="flex:1"> <label class="editor-label">${t('ed.icon')}</label> <select class="editor-emoji-select" onchange="edEmojiSelect(this,'${prefix}.icon',${dayIdx},${cardIdx})"> <option value="">---</option> ${iconOpts} <option value="__custom__" ${isCustomIcon ? 'selected' : ''}>${isCustomIcon ? c.icon + ' ' : ''}${t('ed.customOpt')}</option> </select> </div> </div> <div class="editor-row"> <div class="editor-field" style="flex:1"> <label class="editor-label">${t('ed.startTime')}</label> <input type="time" class="editor-input editor-time-start" id="ed-ts-${dayIdx}-${cardIdx}" value="${timeStart}" onchange="edSetCardTime(${dayIdx},${cardIdx})"> </div> <div class="editor-field" style="flex:1"> <label class="editor-label">${t('ed.endTime')}</label> <input type="time" class="editor-input editor-time-end" value="${timeEnd}" ${!timeStart ? 'disabled' : ''} onchange="edSetCardTime(${dayIdx},${cardIdx})" style="${!timeStart ? 'opacity:0.35;' : ''}"> </div> </div> <div class="editor-field"> <label class="editor-label">${t('ed.title')}</label> <input class="editor-input" value="${escHtml(c.title || '')}" oninput="${prefix}.title=this.value" placeholder="${t('ed.cardTitle')}"> </div>`;

if (isTransport) {
html += ` <div class="editor-row"> <div class="editor-field"><label class="editor-label">${t('ed.from')}</label><input class="editor-input" value="${escHtml(c.from || '')}" oninput="${prefix}.from=this.value" placeholder="${t('ph.fromEx')}"></div> <div class="editor-field"><label class="editor-label">${t('ed.to')}</label><input class="editor-input" value="${escHtml(c.to || '')}" oninput="${prefix}.to=this.value" placeholder="${t('ph.toEx')}"></div> </div> <div class="editor-field"><label class="editor-label">${t('ed.carrier')}</label><input class="editor-input" value="${escHtml(c.carrier || '')}" oninput="${prefix}.carrier=this.value" placeholder="${t('ph.carrierEx')}"></div>`;
}

html += `<div class="editor-field"> <label class="editor-label">${t('ed.subtitleDetails')}</label> <input class="editor-input" value="${escHtml(c.sub || '')}" oninput="${prefix}.sub=this.value" placeholder="${t('ph.stayDetail')}"> </div> ${editorData.days[dayIdx].dayTrip ?`<div class="editor-field">
...
</div>`: ''} ${isTransport ?`<div class="editor-row">
...
</div>`} <div class="editor-row"> <div class="editor-field"><label class="editor-label">${t('ed.phone')}</label><input class="editor-input" value="${escHtml(c.phone || '')}" oninput="${prefix}.phone=this.value" placeholder="+81351557111"></div> <div class="editor-field"><label class="editor-label">${t('ed.email')}</label><input class="editor-input" value="${escHtml(c.email || '')}" oninput="${prefix}.email=this.value" placeholder="info@hotel.com"></div> </div>`;

// ── Tags ──
const tags = parseTags(c.tags);
html += `<div class="editor-subsection" style="margin-top:8px;padding-top:8px"> <div class="editor-subsection-header"><span class="editor-subsection-title">${t('ed.tags')}</span></div>`;
tags.forEach((tg, ti) => {
const tagTypeOpts = TAG_TYPES.map(tt => `<option value="${tt.value}" ${tg.cls === tt.value ? 'selected' : ''}>${tt.label}</option>`).join(’’);
html += `<div class="editor-tag-row"> <input class="editor-inline-input editor-tag-label" value="${escHtml(tg.label)}" oninput="edUpdateTag(${dayIdx},${cardIdx},${ti},'label',this.value)"> <select class="editor-inline-input editor-tag-type" onchange="edUpdateTag(${dayIdx},${cardIdx},${ti},'cls',this.value)">${tagTypeOpts}</select> <button class="editor-remove-btn" onclick="edRemoveTag(${dayIdx},${cardIdx},${ti})">✕</button> </div>`;
});
html += `<button class="editor-add-btn" onclick="edAddTag(${dayIdx},${cardIdx})">${t('ed.addTag')}</button></div>`;

// ── Tips ──
const tips = c.tips || [];
html += `<div class="editor-subsection" style="margin-top:8px;padding-top:8px"> <div class="editor-subsection-header"><span class="editor-subsection-title">${t('ed.tips')}</span></div>`;
tips.forEach((tip, ti) => {
html += `<div class="editor-tip-row"> <input class="editor-inline-input editor-tip-input" value="${escHtml(tip)}" oninput="editorData.days[${dayIdx}].cards[${cardIdx}].tips[${ti}]=this.value"> <button class="editor-remove-btn" onclick="editorData.days[${dayIdx}].cards[${cardIdx}].tips.splice(${ti},1);refreshCardBody(${dayIdx},${cardIdx})">✕</button> </div>`;
});
...
? `<div class="editor-loc-current"> <span class="editor-loc-name">${escHtml(locLabel.split(',').slice(0, 2).join(','))}</span> <span class="editor-loc-coord">${val.lat.toFixed(4)}, ${val.lng.toFixed(4)}</span> <button class="editor-loc-clear" onclick="edClearLocation(${dayIdx},${cardIdx},'${field}')" title="${t('ed.locClear')}">✕</button> </div>`
: ‘’;
...
cur.innerHTML = ` <span class="editor-loc-name">${escHtml(label.split(',').slice(0, 2).join(','))}</span> <span class="editor-loc-coord">${lat.toFixed(4)}, ${lng.toFixed(4)}</span> <button class="editor-loc-clear" onclick="edClearLocation(${dayIdx},${cardIdx},'${field}')" title="${t('ed.locClear')}">✕</button>`;
