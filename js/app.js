...
function initApp() {
  ...
  newTripBtn.textContent                                     = '\u270F\uFE0F New Trip';
  changeTripBtn.textContent                                  = '\u{1F4CB} Change Trip';
  editTripBtn.textContent                                    = '\u270F\uFE0F Edit';
  loadDocsBtn.textContent                                    = '\u{1F4CE} Load Documents';
  document.getElementById('docs-panel-title').textContent    = '\u{1F4CE} Documents';
  selectFolderBtn.textContent                                = '\u{1F4C1} Select Folder';
  selectFilesBtn.textContent                                 = '\u{1F4C4} Select Files';
  document.getElementById('editor-toolbar-icon').textContent = '\u270F\uFE0F';
  document.getElementById('editor-save-btn').textContent     = '\u{1F4BE} Save';

  // Feature pills
  const features = [
    ['\u{1F30F}', 'Timeline view'],
    ['\u{1F4B0}', 'Budget tracking'],
    ['\u{1F4CD}', 'Map links'],
    ['\u{1F4C4}', 'Documents & tickets'],
    ['\u2708\uFE0F', 'Multi-city routes'],
    ['\u{1F3E8}', 'Accommodation'],
    ['\u{1F372}', 'Food & tips'],
    ['\u{1F3F7}\uFE0F', 'Tags & categories']
  ];
  const featuresCt = document.getElementById('picker-features');
...
async function loadTripFromFolder(dirHandle) {
  ...
  if (!jsonFile) {
    alert('No JSON config file found in this folder.\nPlease select a folder containing your trip-config.json file.');
    return;
  }

  const text = await jsonFile.text();
  let data;
  try { data = JSON.parse(text); } catch (err) { alert('Invalid JSON in ' + jsonFileName + ': ' + err.message); return; }
...
function loadTripFromFileList(files) {
  ...
  reader.onload = ev => {
    ...
    } catch (err) { alert('Invalid JSON in ' + jsonFile.name + ': ' + err.message); }
  };
...
function renderDocsList() {
  ...
  if (!ALL_REQUIRED_FILES.length) {
    docsList.innerHTML = '<div style="padding:16px;text-align:center;color:var(--muted)">No documents referenced</div>';
    return;
  }
  ...
  const summary = `<div style="padding:8px 10px;font-size:0.72rem;color:var(--muted);border-bottom:1px solid var(--border)">
    ${loadedCount}/${ALL_REQUIRED_FILES.length} documents loaded
  </div>`;
...
function updateDocsButtonState() {
  ...
  if (total === 0) {
    loadDocsBtn.textContent = '📎 No Docs';
    ...
    if (matched >= total) {
      loadDocsBtn.textContent = `📎 ${total} Loaded ✓`;
      loadDocsBtn.style.borderColor = '#6fcf97';
      loadDocsBtn.style.color = '#6fcf97';
    } else if (matched === 0) {
      loadDocsBtn.textContent = `📎 Load ${total} Doc${total > 1 ? 's' : ''}`;
